...
/**
 * Core Verification State Machine and OAuth Callback Processor
 */
export async function processOAuthCallback(params: {
  ...
  logger.info({ state, ipHash }, 'Processing OAuth2 callback in verification engine');
  ...
  // 4. Check if session was already consumed (Idempotency & Replay Protection)
  if (consumedAt) {
  ...
  // 6. Atomically consume the session to prevent concurrent race conditions
  const consumedSession = await consumeSessionAtomically(sessionId);
  if (!consumedSession) {
    // Concurrent request consumed it
    const existingVerif = await getVerification(sessionUserId, guildId);
    if (existingVerif && existingVerif.status === 'VERIFIED') {
      return { status: 'ALREADY_VERIFIED', guildName: guild.name || 'Discord Server' };
    }
  ...
  // The Discord ID returned by /users/@me IS the canonical truth
  ...
  // Verify that the user verifying matches the session user
  ...
  if (!roleResult.success) {
    // Role assignment failed: row remains FAILED per PRD Section 15
    await updateSessionStatus(sessionId, 'FAILED');
