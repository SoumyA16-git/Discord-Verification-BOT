...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (!member.permissions.has(PermissionFlagsBits.ManageRoles)) {
    await interaction.editReply({ content: 'You lack the **Manage Roles** permission to run this command.' });
    return;
  }
  ...
  try {
    ...
    if (!user || !config) {
      await interaction.editReply({ content: 'Server configuration or user record not initialized.' });
      return;
    }
    ...
    console.error('VERIFY USER CRASH:', err);
    await interaction.editReply({ content: 'Failed to create reverification session.' }).catch(() => null);
  }
}
